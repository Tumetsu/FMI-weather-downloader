...
On Windows, you can package the app to runnable exe with `py2exe` tool by running following command in project root
after installing py2exe:

    `python3 setup.py py2exe`

(TODO: Make Windows batch-script for this or tweak freezing settings on Windows)
...
The dist folder can be transformed into install setup with. [InnoSetup](http://www.jrsoftware.org/isinfo.php) There is a sample script for it which has to be moved into dist folder before compiling the setup.exe.
